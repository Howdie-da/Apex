...
// Modal for creating dynamic groups
// ============================================
...
export const CreateGroupModal: React.FC<CreateGroupModalProps> = ({ currentUserId, onClose, onRoomReady }) => {
  ...
  const toggleUserSelection = (user: User) => {
    ...
    // Reset search so they can search again
    setUsernameInput('');
    setFoundUser(null);
    setHasSearched(false);
  };
          ...
          {!loading && !error && hasSearched && !foundUser && (
            <div className="py-8 text-center select-none">
              <p className="font-mono text-xs text-muted-foreground tracking-wider font-bold">
                No user found matching &ldquo;{usernameInput.trim().replace(/^@/, '')}&rdquo;
              </p>
            </div>
          )}

          {!loading && !error && foundUser && (
            <div className="border border-border/50 bg-background/50 p-3 flex items-center gap-3 transition-colors hover:border-primary/50 hover:bg-card rounded-none cursor-pointer"
              ...
              <div className="shrink-0 flex items-center justify-center w-6 h-6 border border-border/50 bg-background">
                {selectedUsers.some(u => u.id === foundUser.id) && <Check className="w-4 h-4 text-primary" />}
              </div>
            </div>
          )}
        </div>
